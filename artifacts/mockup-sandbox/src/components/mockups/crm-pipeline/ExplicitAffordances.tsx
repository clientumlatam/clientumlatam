...
export default function ExplicitAffordances() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* SIDEBAR WITH VISIBLE BUTTON CHROME */}
        <aside className="w-56 bg-white border-r border-slate-200 overflow-y-auto flex-shrink-0 no-print py-4 px-3 shadow-[1px_0_4px_rgba(0,0,0,0.02)] z-0">
          ...
          {NAV_ITEMS.map((item) => (
            <button
              key={item.id}
              className={`w-full text-left px-3 py-2 rounded-lg flex items-start gap-3 transition-all cursor-pointer mb-0.5 border-2 shadow-sm ${
                item.id === "pipeline" 
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 flex flex-col">
          <div className="flex flex-col gap-8 h-full max-w-[1400px] mx-auto w-full">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
              ...
              {/* KANBAN BOARD WITH EXPLICIT AFFORDANCES */}
                    ...
                    <div key={col.id} className="bg-slate-100/60 rounded-2xl p-3 border border-slate-200/80 flex flex-col gap-3 min-h-[500px]">
                      ...
                      <div className="flex flex-col gap-3 max-h-[650px] overflow-y-auto pr-1 pb-2">
                        {deals.filter((d) => d.stage === col.id).map((deal) => (
                          ...
                          <div 
                            key={deal.id} 
                            className={`group bg-white border-2 border-slate-300 p-3.5 rounded-xl shadow-md flex flex-col gap-3 transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 border-l-4 ${col.borderHover} ${col.borderLeft}`}
                          >
                            <div className="flex justify-between items-start gap-2">
                              ...
                              <button 
                                ...
                                className="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 border-2 border-red-200 hover:border-red-300 px-2 py-1.5 rounded-lg transition-all shadow-sm"
                                title="Eliminar"
                              >
                            ...
                            {col.id === "leads" && (
                              <div className="text-[11px] text-slate-600 flex flex-col gap-1.5 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                            ...
                            {col.id === "bot_contact" && (
                              <div className="text-[11px] bg-emerald-50 text-emerald-800 p-2.5 rounded-lg border border-emerald-200">
                            ...
                            {col.id === "proposed" && (
                              deal.meddicScore ? (
                                <div className={`text-[11px] p-2.5 rounded-lg border ${getMEDDICStatusColor(deal.meddicScore)} font-bold flex items-center justify-between`}>
                                  <span>Score MEDDIC</span>
                                  <span className="font-black font-mono bg-white/50 px-1.5 py-0.5 rounded">{deal.meddicScore}%</span>
                                </div>
                              ) : (
                                <div className="text-[11px] bg-amber-50 text-amber-800 p-2.5 rounded-lg border border-amber-200 font-medium flex gap-2 items-start">
                            ...
                            {col.id === "closed" && (
                              <div className="text-[11px] bg-emerald-100/50 text-emerald-800 p-2.5 rounded-lg border border-emerald-200 font-bold flex items-center gap-2">
                            ...
                            {/* ALWAYS-VISIBLE ACTION CONTROLS WITH LABELS */}
                            <div className="flex flex-col gap-2 border-t-2 border-dashed border-slate-200 pt-3">
                              
                              {/* PRIMARY ACTION - Stage-specific button as full-width prominent control */}
                              {col.id !== "closed" && (
                                <button 
                                  onClick={() => moveDeal(deal.id, "next")} 
                                  className="bg-slate-900 hover:bg-slate-800 text-white text-xs font-extrabold px-4 py-2.5 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 border-2 border-slate-900"
                                >
                                  {col.id === "leads" && "Avanzar a WhatsApp Bot"}
                                  {col.id === "bot_contact" && "Avanzar a Propuesta"}
                                  {col.id === "proposed" && "Marcar como Ganado"}
                              ...
                              <div className="flex gap-2 items-center">
                                ...
                                {col.id !== "leads" && (
                                  <button 
                                    ...
                                    className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 text-[10px] font-bold px-2 py-2 rounded-lg transition-all border-2 border-slate-300 shadow-sm flex items-center justify-center gap-1.5"
                                  >
                                ...
                                {col.id === "leads" && (
                                  <button className="flex-1 bg-blue-100 hover:bg-blue-200 text-blue-800 hover:text-blue-900 text-[10px] font-extrabold px-2 py-2 rounded-lg transition-all border-2 border-blue-300 shadow-sm uppercase tracking-wide">
                                    Evaluar Lead
                                  </button>
                                )}
                                {col.id === "bot_contact" && (
                                  <button className="flex-1 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 hover:text-emerald-900 text-[10px] font-extrabold px-2 py-2 rounded-lg transition-all border-2 border-emerald-300 shadow-sm uppercase tracking-wide">
                                    Outreach
                                  </button>
                                )}
                                {col.id === "proposed" && (
                                  <button className="flex-1 bg-indigo-100 hover:bg-indigo-200 text-indigo-800 hover:text-indigo-900 text-[10px] font-extrabold px-2 py-2 rounded-lg transition-all border-2 border-indigo-300 shadow-sm uppercase tracking-wide">
                                    Score MEDDIC
                                  </button>
                                )}
                              </div>
              ...
              <div className="flex flex-col gap-5 xl:pt-11">
                ...
                {/* Weekly Action Checklist WITH VISIBLE CHECKBOXES */}
                  ...
                  <div className="p-4 pt-3 flex flex-col gap-1.5">
                    {checklist.map((task) => (
                      <button
                        ...
                        className="flex items-start gap-2.5 text-left text-[11px] bg-slate-50 hover:bg-blue-50 p-2.5 rounded-lg transition-all group cursor-pointer border-2 border-slate-200 hover:border-blue-300 shadow-sm"
                      >
                        {task.checked ? (
                          <CheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" />
                        ) : (
                          <div className="w-4 h-4 border-2 border-slate-400 rounded flex-shrink-0 mt-0.5 group-hover:border-blue-500 transition-colors bg-white shadow-sm"></div>
                        )}
