...
import { Megaphone, Plus, MessageCircle, Mail, Linkedin, Play, Pause, BarChart3, Users, CheckCircle2, Clock, AlertCircle, ChevronRight } from 'lucide-react';
...
const INITIAL_CAMPAIGNS: Campaign[] = [
  { id: '1', name: 'Distribuidoras Neuquén Q3', channel: 'whatsapp', status: 'activa', leads: 48, sent: 32, replies: 11, created: '2026-07-10', template: 'intro' },
  { id: '2', name: 'Clínicas & Salud Patagonia', channel: 'email', status: 'borrador', leads: 22, sent: 0, replies: 0, created: '2026-07-16', template: 'cold' },
  { id: '3', name: 'Ferreterías RN junio', channel: 'whatsapp', status: 'completada', leads: 30, sent: 30, replies: 9, created: '2026-06-20', template: 'intro' },
];
...
const CHANNEL_CONFIG: Record<CampaignChannel, { icon: React.ReactNode; label: string; color: string; bg: string }> = {
  ...
  linkedin: { icon: <Linkedin className="w-3.5 h-3.5" />,      label: 'LinkedIn', color: 'text-sky-400',     bg: 'bg-sky-400/10 border-sky-400/30' },
};
...
export default function OutreachCampaigns() {
  ...
  const createCampaign = () => {
    ...
    setCampaigns(prev => [{
      id: Date.now().toString(), name: newName, channel: newChannel,
      status: 'borrador', leads: 0, sent: 0, replies: 0,
      created: new Date().toISOString().split('T')[0], template: newTemplate,
    }, ...prev]);
  ...
  const toggleStatus = (id: string) => {
    setCampaigns(prev => prev.map(c => {
      if (c.id !== id) return c;
      return { ...c, status: c.status === 'activa' ? 'pausada' : c.status === 'pausada' ? 'activa' : c.status };
    }));
  };
  ...
  return (
    <div className="space-y-6 text-slate-200">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total campañas', value: campaigns.length, icon: <Megaphone className="w-4 h-4 text-violet-400" /> },
          { label: 'Leads alcanzados', value: campaigns.reduce((a, c) => a + c.sent, 0), icon: <Users className="w-4 h-4 text-blue-400" /> },
          { label: 'Respuestas', value: campaigns.reduce((a, c) => a + c.replies, 0), icon: <MessageCircle className="w-4 h-4 text-[#10B981]" /> },
          { label: 'Tasa de respuesta', value: `${Math.round(campaigns.reduce((a, c) => a + c.replies, 0) / Math.max(campaigns.reduce((a, c) => a + c.sent, 0), 1) * 100)}%`, icon: <BarChart3 className="w-4 h-4 text-amber-400" /> },
        ].map(s => (
      ...
      {showNew && (
        <div className="bg-[#0D1424] border border-[#10B981]/30 rounded-xl p-5 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={createCampaign} className="px-4 py-2 text-xs font-bold bg-[#10B981] hover:bg-[#059669] text-white rounded-lg transition-all">Crear campaña</button>
          </div>
        </div>
      )}
        ...
        {campaigns.map(c => {
          ...
          return (
            <div key={c.id} className="bg-[#0D1424] border border-[#1E293B] rounded-xl p-4">
              <div className="flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-500">
                    ...
                    <span className="text-slate-400">Tasa: <span className={replyRate > 25 ? 'text-emerald-400 font-bold' : replyRate > 10 ? 'text-amber-400 font-bold' : 'text-rose-400 font-bold'}>{replyRate}%</span></span>
                  </div>
                </div>
              ...
              {c.status === 'activa' && c.sent > 0 && (
                <div className="mt-3">
                  <div className="flex justify-between text-[10px] text-slate-500 mb-1">
                    <span>Progreso</span><span>{c.sent}/{c.leads}</span>
                  </div>
                  <div className="h-1 bg-[#1E293B] rounded-full overflow-hidden">
                    <div className="h-full bg-[#10B981] rounded-full transition-all" style={{ width: `${Math.round(c.sent / c.leads * 100)}%` }} />
                  </div>
                </div>
              )}
            </div>
