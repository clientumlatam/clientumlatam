...
import OutreachCampaigns from "./crm-full/OutreachCampaigns";
import CreacionRapidaCRM from "./crm-full/CreacionRapidaCRM";
import ActividadCRM from "./crm-full/ActividadCRM";
import CopiloIAPanel from "./crm-full/CopiloIAPanel";
import BrochureCRM from "./crm-full/BrochureCRM";
import {
  Users,
  DollarSign,
  Plus,
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#F8FAFC] relative animate-fadeIn h-full font-sans">
      ...
      <div className="flex-1 flex overflow-hidden">
        <aside className="w-[210px] bg-[#0A0E1A] border-r border-[#1A2332] flex-shrink-0 no-print flex flex-col overflow-hidden">
          {/* Scrollable nav */}
              ...
              <div key={group.id} className="mb-1">
                <div className="px-3 pt-2.5 pb-1">
                  <span className="text-[9px] font-bold uppercase tracking-widest text-slate-600 flex items-center gap-1.5">
                    <group.icon className="w-3 h-3 text-slate-600 flex-shrink-0" />
                    {group.label}
                  </span>
                </div>
          ...
          <div className="border-t border-[#1A2332] p-2 flex-shrink-0">
            ...
            >
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-base transition-all ${activeTab === "orquestador" ? "bg-[#10B981]/20" : "bg-[#1A2332]"}`}>🤖</div>
              <div className="min-w-0 text-left">
                <div className="text-[12px] font-semibold leading-tight">Orquestador IA</div>
                <div className={`text-[10px] leading-tight mt-0.5 ${activeTab === "orquestador" ? "text-[#10B981]/70" : "text-slate-600"}`}>Chat con el staff de agentes</div>
              </div>
            </button>
          </div>
        </aside>
...
{/* TAB 2: IDEAL CUSTOMER PROFILE BUILDER */}
        {activeTab === "icp" && (
          <div className="flex-1 w-full">
            <IcpBuilder />
            <div className="hidden bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
        ...
        {/* TAB 7: BROCHURE EDITOR (solo config y pages) */}
        {["config", "pages"].includes(activeTab) && (
