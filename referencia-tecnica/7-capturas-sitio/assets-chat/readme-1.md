...
El repo separado `clientum-agentes` tiene un GitHub Actions workflow (`agentes-clientum.yml`) que corre cada 15 minutos. Lee los GitHub Issues abiertos, los enruta al agente correcto según el label, ejecuta la tarea usando Gemini/Groq/OpenRouter, y cierra el issue con un comentario de resultado.

**Para darle trabajo:** crear un Issue con label `agente:orquestador` y escribir la instrucción en texto libre.

Ver [`setup-y-api-keys.md`](setup-y-api-keys.md) para instrucciones completas de puesta en marcha.
